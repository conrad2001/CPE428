...
def maping(point):
    ...
    actual_y = point[1][0]
    actual_z = point[2][0]
    x = actual_x/actual_z*focal_l+Cx
    y = -actual_y/actual_z*focal_l+Cy
    return (int(x), int(y))

def draw_man(point):
    actial_x, actual_z = point
    actual_y = -2
    man_loca = np.matrix([[actial_x], [actual_y], [actual_z]])
    p1 = man_loca + np.matrix([[0.3], [0.0], [1]])
    p2 = p1 + np.matrix([[0], [1], [0]])
    p3 = man_loca + np.matrix([[0.3], [0.0], [-1]])
    p4 = p3 + np.matrix([[0], [1], [0]])
    p5 = man_loca + np.matrix([[-0.3], [0.0], [1]])
    p6 = p5 + np.matrix([[0], [1], [0]])
    p7 = man_loca + np.matrix([[-0.3], [0.0], [-1]])
    p8 = p7 + np.matrix([[0], [1], [0]])
    cv.line(final_img,maping(p1),maping(p2),(255,0,0),2)
    cv.line(final_img,maping(p1),maping(p3),(255,0,0),2)
    cv.line(final_img,maping(p1),maping(p5),(255,0,0),2)
    cv.line(final_img,maping(p2),maping(p4),(255,0,0),2)
    cv.line(final_img,maping(p2),maping(p6),(255,0,0),2)
    cv.line(final_img,maping(p3),maping(p7),(255,0,0),2)
    cv.line(final_img,maping(p3),maping(p4),(255,0,0),2)
    cv.line(final_img,maping(p4),maping(p8),(255,0,0),2)
    cv.line(final_img,maping(p5),maping(p6),(255,0,0),2)
    cv.line(final_img,maping(p5),maping(p7),(255,0,0),2)
    cv.line(final_img,maping(p6),maping(p8),(255,0,0),2)
    cv.line(final_img,maping(p7),maping(p8),(255,0,0),2)
...
def final():
    ...
    while True:
        ...
        if initBB is None:
            ...
            color = (int(random() * 255), int(random() * 255), int(random() * 255))
            cv.putText(frame, 'Stand 1m away. Place your head in the box!', (30, 40), cv.FONT_HERSHEY_SIMPLEX, 0.8, color, 2,
                       cv.LINE_AA)
            cv.rectangle(frame, (center_x - head_size, center_y - head_size), (center_x + head_size, center_y + head_size),
                         color, 2)
        ...
        if man_pos is not None and man_pos[0] < screen_size[0] and man_pos[1] < screen_size[1]:
            man_on_img_pos = map_man_2_img(man_pos, (origin_frame_x, origin_frame_y), (-10, 10, 100))
            # print(man_on_img_pos)
            draw_man(man_on_img_pos)  # accept(x,y), x: left/right   y: front/back   z: hight
...
def map_man_2_img(man_pos, original_map, final_map):
    ...
    final_x1,final_x2,final_y = final_map
    return (int(man_x*(final_x2-final_x1)/ori_x+final_x1),int(man_y*final_y/ori_y))
...
def temp():
    ...
    player_img = get_player_img(focal_l, Cx, Cy, cap)

    sift = cv.xfeatures2d.SIFT_create()
    player_img_gray = cv.cvtColor(player_img, cv.COLOR_BGR2GRAY)
    kp1, des1 = sift.detectAndCompute(player_img_gray, None)  # get key point and descriptor

    while 1:
        ret, frame = cap.read()
        man_pos = get_man_pos(player_img, frame, kp1, des1, sift)
        origin_frame_y, origin_frame_x, _ = frame.shape
        frame = cv.resize(frame,screen_size)
        final_img = copy.copy(background_img)
