...
def final():
    ...
    video = cv.VideoWriter(filename=('SIFT_evaluation.avi'), fourcc=cv.VideoWriter_fourcc('M', 'P', 'E', 'G'),
                           fps=12,
                           frameSize=screen_size)
    vs = cv.VideoCapture(r'C:\Users\User01\PycharmProjects\CPE428\evaluation\SIFT_detect.avi')


    while playing:
        ret, frame = vs.read() # check to see if we have reached the end of the stream
        if frame is None:
            break

        frame = cv.resize(frame, screen_size)
        key = cv.waitKey(1) & 0xFF
        if key == ord("q") or frame_count == 500:
            break
    ...
    np.savetxt("evaluate_pos.csv", pos, delimiter=",")
